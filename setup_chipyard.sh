...
wget https://github.com/conda-forge/miniforge/releases/download/25.11.0-1/Miniforge3-Linux-x86_64.sh -O ~/.miniforge3.sh
...
echo source ~/conda/etc/profile.d/conda.sh > .env.sh
cat env.sh >> .env.sh
mv .env.sh env.sh
source env.sh

make -C generators/gemmini/software/libgemmini install
make -C sims/verilator CONFIG=GemminiRocketConfig -j 4

cd generators/gemmini
sed -i 's/chipyard-CustomGemminiSoCConfig/chipyard.harness-GemminiRocketConfig/g' scripts/run-verilator.sh

cd software/gemmini-rocc-tests

cat > bareMetalC/tiled_matmul_ws.c <<'EOF'
